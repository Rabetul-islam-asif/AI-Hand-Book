...
### Chapter Goal
এই চ্যাপ্টারের মূল লক্ষ্য হলো কৃত্রিম Neural Network-এর সবচেয়ে শক্তিশালী ও অলৌকিক Engine— মানে Backpropagation (Backpropagation) এবং চেইন রুল (Chain Rule) এর Math-এর কুয়াশা পুরোপুরি পরিষ্কার করা। আমরা দেখবো কীভাবে Forward পাসের (Forward Pass) মাধ্যমে নেটওয়ার্ক Input থেকে Output Predict করে, কীভাবে Error হিসেব করে এবং last চেইন রুল Calculus-এর মাধ্যমে ব্যাকওয়ার্ড পাসে (Backward Pass) প্রতিটি স্তরের ওয়েইট আপডেট করে ভুল শুধরে নেয়।

### Why Should I Care?
Deep Learning-এর দুনিয়ায় Backpropagation ছাড়া কোনো Model এক বিন্দুও শিখতে পারতো না। তুমি যখন কোডে `loss.backward()` কল করো, পর্দার আড়ালে Calculus-এর কোটি কোটি Equation চেইন রুল দিয়ে নিমিষেই সলভ হয়ে যায়। এই চ্যাপ্টারটি পড়লে তুমি একজন "ব্ল্যাক বক্স কলার" থেকে একজন "হোয়াইট বক্স Architect"-এ convertিত হবে, যিনি Neural Network-এর ভেতরের প্রতিটি Pixel Data ফ্লো Math-এরভাবে ফিল করতে পারো।

### Big Picture
আমরা আগের চ্যাপ্টারে একক নিউরন বা Perceptronের Mechanics ও Activation Function দেখেছি। এই চ্যাপ্টারে আমরা সেই একক নিউরনগুলোকে স্তরে স্তরে সাজিয়ে একটি ডিপ ফিডForward নেটওয়ার্ক (Deep Feedforward Network) বা মাল্টি-লেয়ার Perceptron (MLP) তৈরি করবো এবং এর সম্পূর্ণ Training Mechanism স্ক্র্যাচ Code দিয়ে ভিজুয়ালাইজ করবো।
