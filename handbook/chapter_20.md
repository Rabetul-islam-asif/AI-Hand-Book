...
### Chapter Goal
এই চ্যাপ্টারের মূল লক্ষ্য হলো এআই টুল এবং রিসোর্স ইন্টিগ্রেশনের সবচেয়ে আধুনিক ও বৈপ্লবিক ওপেন-স্ট্যান্ডার্ড—অর্থাৎ Model Context প্রোটোকল (Model Context Protocol / MCP) এর Mechanism এবং তার Coding Architecture সম্পূর্ণ আয়ত্ত করা। তুমি জানতে পারবে কীভাবে অ্যানথ্রপিক (Anthropic) এর তৈরি করা এই ইউনিভার্সাল প্রোটোকল এআই হোস্ট (যেমন: Claude, Cursor) এবং বিভিন্ন Data সোর্সের মধ্যে একটি কমন কানেকশন ব্রিজ তৈরি করে এবং কীভাবে নিজের কাস্টম **MCP Server** ডেভেলপ ও ডিপ্লয় করতে হয়।

### Why Should I Care?
আগের চ্যাপ্টারে আমরা কাস্টম টুল কলিং শিখেছি। কিন্তু প্রথাগত টুল কলিংয়ের সবচেয়ে বড় সমস্যা হলো এটি Model-স্পেসিফিক (Model-specific)। তুমি যদি Claude (Claude) এর জন্য টুল ডিফাইন করো, সেটি ওপেনএআই (OpenAI) বা Gemini (Gemini)-তে কাজ করবে না। তোমাকে প্রতিটির জন্য আলাদা API ফরম্যাট ও Code লিখতে হবে। **MCP হলো এআই-এর জন্য USB-C ক্যাবলের মতো।** তুমি একবার একটি কাস্টম MCP Server তৈরি করলে, যেকোনো এআই হোস্ট বা Model সেই একই Server কানেক্ট করে ডাইরেক্ট টুলস ও রিসোর্স রিড করতে পারবে। এটি বর্তমান গ্লোবাল এআই ইনফ্রাস্ট্রাকচারের সবচেয়ে মডার্ন আর্কিটেকচারাল Pattern।
...
* **The Old Mess (নন-স্ট্যান্ডার্ড চার্জার):** নোকিয়া, স্যামসাং, সনি এরিকসন—প্রতিটি ফোনের জন্য আলাদা চিকন পিন, মোটা পিন বা চ্যাপ্টা পিনের চার্জার লাগতো। এক চার্জার অন্য ফোনে কাজ করতো না। Developer হিসেবে প্রতিটি এআই মডেলে আলাদা কাস্টম Coding করে টুল কলিং জোড়া দেওয়া ছিল ঠিক এই পুরানো চার্জার বিশৃঙ্খলার মতো।
...
Model Context প্রোটোকল (MCP) হলো এআই-এর সেই **USB-C** স্ট্যান্ডার্ড। এটি একবার Data বা টুলস এক্সপোজ করে এবং যেকোনো এলএলএম হোস্ট ও ক্লায়েন্ট সেই Data রিড ও প্রসেস করতে পারে।
...
#### ক. MCP Host (Client)
যেকোনো এআই Application বা এডিটর যা প্রোটোকলটি সাপোর্ট করে (যেমন: Claude Desktop App, Cursor, Zed Editor, or your custom LLM app)। ক্লায়েন্ট সার্ভারকে বলে: *"তোমার কাছে কী কী টুল বা রিসোর্স আছে তার লিস্ট দাও।"*
...
1. **Resources (উৎস):** এআই রিড করতে পারে এমন যেকোনো Data। যেমন: তোমার ডকার File, Postgres Database টেবিল বা কাস্টম ওয়েব পেজ কনটেন্ট।
2. **Prompts (Prompt টেমপ্লেট):** আগে থেকে ডিজাইন করা System Prompt বা ইউজার গাইড।
3. **Tools (Function):** এআই এক্সেকিউট করতে পারে এমন কাস্টম Code। যেমন: File রাইটার, ব্যাশ রানার বা কাস্টম Database কোয়েরিয়ার।
...
2. **Directory Map:** এমসিপি সার্ভারটি উইন্ডোজ ওএস-এর পাথ রিড করে রিসোর্স হিসেবে Vector ম্যাপ হোস্ট ক্লায়েন্টকে পাস করে।
3. **Auto Search:** এআই হোস্টটি সেই রিসোর্স এনালাইসিস করে সরাসরি তোমার কম্পিউটারের ফাইলে Code চেঞ্জ ও ব্যাশ Test রান করতে পারে, যা আগে প্রতিটি মডেলে আলাদা স্ক্রিপ্ট ছাড়া অসম্ভব ছিল।
...
# ৩. কাস্টম এমসিপি রিসোর্স রেজিস্টার করো (Static Data/Logs)
...
* **The Security Threat:** যেহেতু এমসিপি Server সাধারণত `stdio` (Standard Input/Output) পাইপ ব্যবহার করে হোস্টের রুট পারমিশনে উইন্ডোজ ওএস কমান্ড বা ব্যাশ রান করে, তাই একটি ক্ষতিকর এআই Prompt জেইলব্রেকের মাধ্যমে এমসিপি Server হ্যাক করে তোমার পুরো ওএস ড্যামেজ করে দিতে পারে।
...
**ভুল ধারণা:** এমসিপি Server Code লেখার পর প্রতিবার এআই Model পরিবর্তন করার সময় Server Code আবার নতুন করে এআই মডেলে রেজিস্টার করতে হয়।

**বাস্তবতা:** এমসিপি Server সম্পূর্ণ স্বায়ত্তশাসিত। ক্লায়েন্ট বা হোস্ট এডিটর (যেমন: Cursor) যখন তোমার ওএস-এর `mcpConfig.json` রিড করে রান হয়, সে নিজে থেকেই সার্ভারকে কুয়্যারি করে তার এভেলেবল টুলস অটো-ডিসকভার (Auto-discover) করে নেয়। তোমার এআই Code ফাইলে কোনো ম্যানুয়াল ইন্টিগ্রেশন করতে হয় না।
...
ভাবো তোমার কম্পিউটারটি হলো এআই হোস্ট (Claude/Cursor)। তুমি কম্পিউটারে আলাদা কাস্টম পোর্ট বানাতে চান না। 
...
1. **প্রশ্ন:** প্রথাগত Model-স্পেসিফিক টুল কলিংয়ের তুলনায় Model Context প্রোটোকল (MCP) এর প্রধান বৈপ্লবিক সুবিধা কী?
   * **উত্তর:** প্রথাগত টুল কলিং Model-স্পেসিফিক (যেমন: ক্লডের জন্য লেখা Code জিপিটি-তে চলে না)। কিন্তু MCP হলো একটি ওপেন-স্ট্যান্ডার্ড ইউনিভার্সাল প্রোটোকল, যা একবার Server তৈরি করলে যেকোনো এআই হোস্ট বা এডিটর (Claude Desktop, Cursor, etc.) ডাইরেক্ট ড্রাইভার ছাড়াই টুল ডিসকভার ও কল করতে পারে।

#### Intermediate
2. **প্রশ্ন:** MCP প্রোটোকলের তিনটি প্রধান স্তম্ভ (Three Pillars) কোনগুলো এবং এদের কাজ কী?
   * **উত্তর:** প্রথমত, **Resources** (রিড-অনলি Data যেমন ওএস File, Database)। দ্বিতীয়ত, **Prompts** (System Prompt টেমপ্লেট)। তৃতীয়ত, **Tools** (এআই এক্সেকিউট করতে পারে এমন কাস্টম ব্যাশ বা Database Function)।
...
### ১০. Chapter Summary
* **MCP** হলো এআই টুল ও Data ইন্টিগ্রেশনের ওপেন-স্ট্যান্ডার্ড ইউনিভার্সাল চার্জার (USB-C)।
