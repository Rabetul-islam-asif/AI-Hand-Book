...
### Chapter Goal
এই চ্যাপ্টারের মূল লক্ষ্য হলো এআই টুল এবং রিসোর্স ইন্টিগ্রেশনের সবচেয়ে আধুনিক ও বৈপ্লবিক ওপেন-স্ট্যান্ডার্ড—অর্থাৎ মডেল কনটেক্সট প্রোটোকল (Model Context Protocol / MCP) এর মেকানিজম এবং তার কোডিং আর্কিটেকচার সম্পূর্ণ আয়ত্ত করা। তুমি জানতে পারবে কীভাবে অ্যানথ্রপিক (Anthropic) এর তৈরি করা এই ইউনিভার্সাল প্রোটোকল এআই হোস্ট (যেমন: Claude, Cursor) এবং বিভিন্ন ডেটা সোর্সের মধ্যে একটি কমন কানেকশন ব্রিজ তৈরি করে এবং কীভাবে নিজের কাস্টম **MCP Server** ডেভেলপ ও ডিপ্লয় করতে হয়।

### Why Should I Care?
আগের চ্যাপ্টারে আমরা কাস্টম টুল কলিং শিখেছি। কিন্তু প্রথাগত টুল কলিংয়ের সবচেয়ে বড় সমস্যা হলো এটি মডেল-স্পেসিফিক (Model-specific)। তুমি যদি ক্লড (Claude) এর জন্য টুল ডিফাইন করো, সেটি ওপেনএআই (OpenAI) বা জেমিনি (Gemini)-তে কাজ করবে না। তোমাকে প্রতিটির জন্য আলাদা এপিআই ফরম্যাট ও কোড লিখতে হবে। **MCP হলো এআই-এর জন্য USB-C ক্যাবলের মতো।** তুমি একবার একটি কাস্টম MCP Server তৈরি করলে, যেকোনো এআই হোস্ট বা মডেল সেই একই সার্ভার কানেক্ট করে ডাইরেক্ট টুলস ও রিসোর্স রিড করতে পারবে। এটি বর্তমান গ্লোবাল এআই ইনফ্রাস্ট্রাকচারের সবচেয়ে মডার্ন আর্কিটেকচারাল প্যাটার্ন।
...
* **The USB-C Standard (MCP):** এখন বাজারে এলো USB-C ক্যাবল। তুমি ল্যাপটপ, ফোন, ট্যাবলেট—যেকোনো ডিভাইসে একটিমাত্র ক্যাবল প্লাগ-ইন করে চার্জ ও ডেটা শেয়ার করতে পারো। 
...
#### খ. MCP Server
এটি তোমার লোকাল কম্পিউটার বা ক্লাউড সার্ভারে চলা একটি ছোট প্রসেস যা ডেটা এবং টুল এক্সপোজ করে। এটি ক্লায়েন্টের সাথে **JSON-RPC 2.0** প্রোটোকল ব্যবহার করে স্টাইলিশ উপায়ে কমিউনিকেট করে (Over `stdio` or `Server-Sent Events - SSE`)।

#### গ. The Three Pillars of MCP (তিনটি প্রধান রিসোর্স টাইপ)

1. **Resources (উৎস):** এআই রিড করতে পারে এমন যেকোনো ডেটা। যেমন: তোমার ডকার ফাইল, পোস্টগ্রেস ডাটাবেস টেবিল বা কাস্টম ওয়েব পেজ কনটেন্ট।
...
**MCP is Open Source!**  
অ্যানথ্রপিক এটি তৈরি করলেও এটি সম্পূর্ণ ওপেন স্ট্যান্ডার্ড। জেমিনি বা জিপিটি ডেভেলপাররাও এই একই প্রোটোকল ফ্রেমওয়ার্ক ব্যবহার করে কাস্টম আরএজি ও এজেন্ট কোডবেস ইন্টিগ্রেট করতে পারো।

---

### ৩. Real World Example: Cursor-এর গ্লোবাল টুল ইন্টিগ্রেশন

Cursor বা Claude Desktop-এ যখন তুমি কাস্টম ফিচার ফাইল সার্চ করতে চান:
...
3. **Auto Search:** এআই হোস্টটি সেই রিসোর্স এনালাইসিস করে সরাসরি তোমার কম্পিউটারের ফাইলে কোড চেঞ্জ ও ব্যাশ টেস্ট রান করতে পারে, যা আগে প্রতিটি মডেলে আলাদা স্ক্রিপ্ট ছাড়া অসম্ভব ছিল।
...
# ৪. সার্ভার রান করো (Over stdio standard)
...
* **The Security Threat:** যেহেতু এমসিপি সার্ভার সাধারণত `stdio` (Standard Input/Output) পাইপ ব্যবহার করে হোস্টের রুট পারমিশনে উইন্ডোজ ওএস কমান্ড বা ব্যাশ রান করে, তাই একটি ক্ষতিকর এআই প্রম্পট জেইলব্রেকের মাধ্যমে এমসিপি সার্ভার হ্যাক করে তোমার পুরো ওএস ড্যামেজ করে দিতে পারে।
...
**বাস্তবতা:** এমসিপি সার্ভার সম্পূর্ণ স্বায়ত্তশাসিত। ক্লায়েন্ট বা হোস্ট এডিটর (যেমন: Cursor) যখন তোমার ওএস-এর `mcpConfig.json` রিড করে রান হয়, সে নিজে থেকেই সার্ভারকে কুয়্যারি করে তার এভেলেবল টুলস অটো-ডিসকভার (Auto-discover) করে নেয়। তোমার এআই কোড ফাইলে কোনো ম্যানুয়াল ইন্টিগ্রেশন করতে হয় না।
...
চলো পাইথনে কোনো কাস্টম লাইব্রেরি ছাড়া স্ক্র্যাচ থেকে একটি এমসিপি সার্ভারের ইন্টারনাল `stdio JSON-RPC` প্রোটোকল হ্যান্ডশেক ও টুল এক্সেকিউশন লুপ ডেভেলপ করি।
...
#### Advanced
3. **প্রশ্ন:** এন্টারপ্রাইজ প্রোডাকশনে stdio-ভিত্তিক MCP সার্ভার ডিপ্লয় করার প্রধান সিকিউরিটি থ্রেট কী এবং এর স্ট্যান্ডার্ড মিটিগেশন স্ট্র্যাটেজি ব্যাখ্যা করো।
