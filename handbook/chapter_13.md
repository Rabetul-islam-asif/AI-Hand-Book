...
### Chapter Goal
এই চ্যাপ্টারের মূল লক্ষ্য হলো এলএলএম-এর Generative ক্ষমতার সাথে Custom নলেজ বেসের মেলবন্ধন ঘটানোর পাইপলাইন— মানে আরএজি (RAG - Retrieval-Augmented Generation) এর কোর Mechanism perfectly বোঝা। তুমি জানতে পারবে কীভাবে আরএজি-র ইনজেস্ট (Ingestion), রিট্রিভাল (Retrieval), এবং জেনারেশন (Generation) পাইপলাইন কাজ করে এবং বিভিন্ন প্রকার চাংকিং Strategy (Chunking - Character, Token, Recursive, Semantic) এর Practical প্রয়োগ ও প্রোডাকশন প্রজেক্টে এগুলোর perfect সিলেকশন Criteria আয়ত্ত করতে পারবে।

### Why Should I Care?
ChatGPT বা Gemini যতই শক্তিশালী হোক না কেন, তারা তোমার Companyর ইন্টারনাল পলিসি, Customারের লাইভ ট্রানজ্যাকশন বা আজ সকালের কোনো সিক্রেট আপডেট জানে না। Model Fine-Tuning করা খুব ব্যয়বহুল এবং সময়সাপেক্ষ। আরএজি হলো সবচেয়ে সস্তা, নিরাপদ এবং রিয়েল-টাইম মেথড যা মডেলকে সাথে সাথে নতুন তথ্য শিখিয়ে কাজ করাতে পারে। আরএজি পাইপলাইন অপ্টিমাইজেশন না জানলে তুমি কখনই Enterprise-Grade AI প্রডাক্ট Architect করতে পারবে না।

### Big Picture
আগের চ্যাপ্টারগুলোতে আমরা Vector Database এবং Embeddingsের Math-এর Mechanism শিখেছি। এই চ্যাপ্টার থেকে আমাদের শুরু হচ্ছে **RAG (Retrieval-Augmented Generation)** এর world। এখানে শেখা চাংকিং এবং রিট্রিভাল ভৌত অবকাঠামো আমাদের পরের chapter-এর অ্যাডভান্সড হাইব্রিড Search এবং রির‍্যাঙ্কিং বোঝার মূল ভিত্তি।
...
* **Open-Book Exam (RAG):** শিক্ষক তোমাকে পরীক্ষার হলে সম্পূর্ণ বুক এবং Companyর পলিসি File সাথে নিয়ে বসার অনুমতি দিলে (Open-Book Exam)। তোমার কাজ হলো প্রশ্নটি পড়ে প্রথমে বইয়ের সূচিপত্র ঘেঁটে সঠিক চ্যাপ্টার ও প্যারাগ্রাফটি খুঁজে বের করা (Retrieval), তারপর সেই পৃষ্ঠাটি চোখের সামনে খোলা রেখে perfect ও সত্যবাদী উত্তর খসড়া করা (Generation)। 
...
একটি পুরো আরএজি Architecture মূলত দুটি আলাদা পাইপলাইনে কাজ করে:
...
3. **Embedding:** প্রতিটি ছোট খণ্ড বা চাঙ্ককে Vector-এ convert করা।
4. **Vector Database Store:** Vectorগুলোকে ফাস্ট সার্চের জন্য Indexিং করে সেভ রাখা।
...
চাংক করার সময় আমরা সর্বদা দুটি চাঙ্কের মাঝখানে কিছুটা ওভারল্যাপ (Overlap) রাখি। এর Mechanismটি নিচে Diagramের মাধ্যমে ভিজ্যুয়ালাইজ করো:
...
পাইথনে `langchain` Library ব্যবহার করে Custom রিকিউরসিভ স্প্লিটার ও চাঙ্ক ওভারল্যাপ ডিফাইন করার রিয়েল Code:
...
চলো পাইথনে Custom NumPy ব্যবহার করে কোনো এমএল ফ্রেমওয়ার্ক ছাড়া একটি পুরো মিনি আরএজি রিট্রিভাল ও Inference Context ইনজেক্টর System Architect করি।
...
#### Code Breakdown:
* **Input:** Customার কুয়্যারি Vector এবং ৩-ডাইমেনশন পলিসি Embeddings।
...
1. **প্রশ্ন:** আরএজি (RAG) কী এবং AI চ্যাটবটে এটি কেন ব্যবহার করা হয়?
   * **উত্তর:** RAG হলো Retrieval-Augmented Generation। এটি Model-এর ইন্টারনাল ওয়েটসের ওপর নির্ভর না করে External নলেজ বেস (Vector Database) থেকে রিয়েল-টাইম সঠিক তথ্য খুঁজে এনে Prompt-এর সাথে ইনজেক্ট করে উত্তর জেনারেট করার মেথড। এটি Hallucination কমাতে ও Companyর Custom Data মডেলে Integrate করতে সাহায্য করে।
...
* **Ingestion Pipeline** অফলাইনে চাংকিং ও Embeddings করে Vector Database প্রস্তুত করে।
