...
### Chapter Goal
এই চ্যাপ্টারের মূল লক্ষ্য হলো এলএলএম-এর জেনারেটিভ ক্ষমতার সাথে কাস্টম নলেজ বেসের মেলবন্ধন ঘটানোর পাইপলাইন—অর্থাৎ আরএজি (RAG - Retrieval-Augmented Generation) এর কোর Mechanism নিখুঁতভাবে বোঝা। তুমি জানতে পারবে কীভাবে আরএজি-র ইনজেস্ট (Ingestion), রিট্রিভাল (Retrieval), এবং জেনারেশন (Generation) পাইপলাইন কাজ করে এবং বিভিন্ন প্রকার চাংকিং স্ট্র্যাটেজি (Chunking - Character, Token, Recursive, Semantic) এর প্র্যাক্টিক্যাল প্রয়োগ ও প্রোডাকশন প্রজেক্টে এগুলোর নিখুঁত সিলেকশন ক্রাইটেরিয়া আয়ত্ত করতে পারবে।

### Why Should I Care?
ChatGPT বা Gemini যতই শক্তিশালী হোক না কেন, তারা তোমার কোম্পানির ইন্টারনাল পলিসি, কাস্টমারের লাইভ ট্রানজ্যাকশন বা আজ সকালের কোনো সিক্রেট আপডেট জানে না। Model Fine-Tuning করা অত্যন্ত ব্যয়বহুল এবং সময়সাপেক্ষ। আরএজি হলো সবচেয়ে সস্তা, নিরাপদ এবং রিয়েল-টাইম মেথড যা মডেলকে তাৎক্ষণিকভাবে নতুন তথ্য শিখিয়ে কাজ করাতে পারে। আরএজি পাইপলাইন অপ্টিমাইজেশন না জানলে তুমি কখনই Enterprise-Grade AI প্রডাক্ট আর্কিটেক্ট করতে পারবে না।

### Big Picture
আগের চ্যাপ্টারগুলোতে আমরা Vector Database এবং এম্বেডিংসের গাণিতিক Mechanism শিখেছি। এই চ্যাপ্টার থেকে আমাদের শুরু হচ্ছে **RAG (Retrieval-Augmented Generation)** এর মহাবিশ্ব। এখানে শেখা চাংকিং এবং রিট্রিভাল ভৌত অবকাঠামো আমাদের পরবর্তী চ্যাপ্টারের অ্যাডভান্সড হাইব্রিড Search এবং রির‍্যাঙ্কিং বোঝার মূল ভিত্তি।

---

### ১. Hook: পরীক্ষার হলে বই খুলে পরীক্ষা দেওয়ার স্বাধীনতা

কল্পনা করো, তুমি একটি অত্যন্ত কঠিন ফাইনাল পরীক্ষা দিতে বসেছো। 
* **Closed-Book Exam (Fine-Tuning বা বেস Model):** তোমাকে পরীক্ষার হলে কোনো বই বা খাতা নিতে দেওয়া হলো না। তোমাকে তোমার মস্তিষ্কে সেভ থাকা পুরনো মুখস্থ নলেজ (Trained Weights) ব্যবহার করে সব কঠিন প্রশ্নের উত্তর লিখতে হবে। প্রশ্ন যদি তোমার সিলেবাসের বাইরে আসে, তবে তুমি বানিয়ে ভুল উত্তর (Hallucination) লিখবে।
...
একটি পূর্ণাঙ্গ আরএজি Architecture মূলত দুটি আলাদা পাইপলাইনে কাজ করে:
...
1. **Document Loading:** পিডিএফ, ওয়ার্ড File বা Database থেকে র টেক্সট রিড করা।
2. **Chunking (খন্ডন):** টেক্সট ফাইলটি অনেক বড় হলে (যেমন: ৫০ পৃষ্ঠার ম্যানুয়াল) Model-এর Context উইন্ডোতে আটবে না। তাই টেক্সটকে ছোট ছোট খণ্ডে বিভক্ত করা হয়।
...
1. **Retrieve:** ইউজারের প্রশ্নটি এম্বেড করে Vector Database থেকে টপ $K$ সংখ্যক (যেমন: সেরা ৩টি) রিলেটেড চাঙ্ক খুঁজে আনা।
2. **Augment:** সেই ৩টি চাঙ্ক Prompt-এর সাথে Context হিসেবে জোড়া লাগানো।
3. **Generate:** এলএলএম Model সেই ইনজেক্টেড Context পড়ে ফ্যাট-ভিত্তিক সত্য উত্তর লিখে দেয়।

#### গ. Chunking Strategies (খন্ডন কৌশল)
আরএজি-র সাফল্যের ৮০% নির্ভর করে তুমি কীভাবে Data খণ্ড বা চাঙ্ক করছো তার ওপর:

1. **Character Chunking (ক্যারেক্টার খন্ডন):** প্রতি ৫০০ ক্যারেক্টার পর পর কেটে দেওয়া। 
   * **সমস্যা:** Conditional লাইনের মাঝখানে কেটে যাওয়ায় অর্থ নষ্ট হয়।
2. **Token Chunking (Token খন্ডন):** প্রতি ২০০ Token পর পর কাটা। কস্ট কন্ট্রোলের জন্য ভালো, তবে অর্থগত Loss হতে পারে।
3. **Recursive Character Chunking (পুনরাবৃত্তিমূলক খন্ডন):** এটি ইন্ডাস্ট্রির সবচেয়ে প্রিয় ও স্ট্যান্ডার্ড পদ্ধতি। এটি প্রথমে Paragraph (`\n\n`), তারপর লাইন (`\n`), তারপর স্পেস দেখে ইন্টেলিজেন্টলি কাটে যাতে বাক্যের অর্থ ও প্যারাগ্রাফের পূর্ণতা বজায় থাকে।
4. **Semantic Chunking (অর্থগত খন্ডন):** এটি সবচেয়ে অ্যাডভান্সড পদ্ধতি। এটি পুরো টেক্সটের এম্বেডিংস Vector-এর পরিবর্তন পরিমাপ করে। যখনই টেক্সটের টপিক বা মিনিং চেঞ্জ হয় (Cosine distance বাড়ে), সে নতুন চাঙ্ক তৈরি করে।
...
Perplexity.ai বা চ্যাটজিপিটির ব্রাউজিং Feature যেভাবে উত্তর জেনারেট করে:

1. **Semantic Chunking:** তারা Search Engine থেকে পাওয়া ৫টি ওয়েব পেজ স্ক্র্যাপ করে সেগুলোর সিমান্টিক চাঙ্ক তৈরি করে।
2. **Cosine Retrieval:** তোমার কুয়্যারির সাথে যে চাঙ্কগুলোর মিল ৯০%-এর বেশি, সেগুলো ছেঁকে নিয়ে প্রম্পটে সাজায়।
3. **Context Injection & Citation:** Prompt-এর ভেতর কড়া System রুল থাকে: `"Answer only using the context below and cite your sources [1], [2]..."`। এর ফলে AI হ্যালুসিনেশনহীন নিখুঁত উত্তর ও সাইটেশন প্রডিউস করে।
...
পাইথনে `langchain` Library ব্যবহার করে কাস্টম রিকিউরসিভ স্প্লিটার ও চাঙ্ক ওভারল্যাপ ডিফাইন করার রিয়েল Code:
...
* **Context Leakage:** একজন সাধারণ কাস্টমার চ্যাটবটে এমন প্রশ্ন করল যা কোম্পানির ইন্টারনাল AI হ্যাক করে কোনো অ্যাডমিনের স্যালারি শিট বা সিক্রেট Project-এর Vector রিট্রাইভ করে প্রম্পটে ইনজেক্ট করে ফেলল।
...
**ভুল ধারণা:** আরএজি প্রজেক্টে চাঙ্ক সাইজ যত বড় রাখা যাবে, Context তত বেশি মডেলে পৌঁছাবে এবং উত্তর তত ভালো হবে।
...
আরএজি পাইপলাইনের মেন্টাল Model:
...
চলো পাইথনে কাস্টম NumPy ব্যবহার করে কোনো এমএল ফ্রেমওয়ার্ক ছাড়া একটি পূর্ণাঙ্গ মিনি আরএজি রিট্রিভাল ও Inference Context ইনজেক্টর System আর্কিটেক্ট করি।
...
#### Code Breakdown:
* **Input:** কাস্টমার কুয়্যারি Vector এবং ৩-ডাইমেনশন পলিসি এম্বেডিংস।
* **Output:** বেস্ট ম্যাচ ডক রিট্রিভ করে জেনারেট হওয়া ফাইনাল AI ইনজেকশন Prompt।
* **Why it works:** Vector সিমিলারিটি কোসাইন মেপে `visit the nearest bKash center` ডকটি নির্ভুলভাবে রিট্রাইভ করেছে এবং প্রম্পটে ইনজেক্ট করেছে।
* **When to use:** কাস্টম আরএজি (RAG) Prompt বিল্ডিং ও রিট্রিভাল পাইপলাইন স্ক্র্যাচ থেকে ডিজাইন করার জন্য।
...
#### Beginner
1. **প্রশ্ন:** আরএজি (RAG) কী এবং AI চ্যাটবটে এটি কেন ব্যবহার করা হয়?
   * **উত্তর:** RAG হলো Retrieval-Augmented Generation। এটি Model-এর ইন্টারনাল ওয়েটসের ওপর নির্ভর না করে এক্সটার্নাল নলেজ বেস (Vector Database) থেকে রিয়েল-টাইম সঠিক তথ্য খুঁজে এনে Prompt-এর সাথে ইনজেক্ট করে উত্তর জেনারেট করার মেথড। এটি হ্যালুসিনেশন কমাতে ও কোম্পানির কাস্টম Data মডেলে ইন্টিগ্রেট করতে সাহায্য করে।

#### Intermediate
2. **প্রশ্ন:** Recursive Character Chunking কেন সাধারণ ক্যারেক্টার চাংকিংয়ের চেয়ে বেশি কার্যকর?
   * **উত্তর:** সাধারণ ক্যারেক্টার চাংকিং যেকোনো জায়গায় Conditional লাইনের মাঝখানে কেটে ফেলে বাক্যের অর্থ নষ্ট করে। কিন্তু Recursive Character Splitter নির্দিষ্ট অগ্রাধিকার তালিকায় Paragraph, লাইন এবং স্পেস দেখে ইন্টেলিজেন্টলি কাটে, যা প্রতিটি খণ্ডের অর্থগত পূর্ণতা অক্ষুণ্ণ রাখে।
...
### ১১. Chapter Summary
* **RAG** ক্লোজড-বুক AI মডেলকে একটি ওপেন-বুক ফ্যাট-বেসড AI সিস্টেমে রূপান্তর করে।
* **Ingestion Pipeline** অফলাইনে চাংকিং ও এম্বেডিংস করে Vector Database প্রস্তুত করে।
* **Recursive Chunking** এবং **Overlap** ডকের সীমানাগত অর্থ রক্ষা করার জন্য সবচেয়ে গুরুত্বপূর্ণ।
* প্রোডাকশন সিস্টেমে Data সিকিউরিটি নিশ্চিত করতে মেটাডেটা **ACL Filtering** ব্যবহার করা আবশ্যিক।

---

### ১২. What's Next
দারুণ! আমরা সফলভাবে আরএজি-র কোর ফাউন্ডেশন এবং চাংকিং মেকানিক্স জয় করে ফেলেছি। পরবর্তী চ্যাপ্টারে আমরা এই রিট্রিভাল পাইপলাইনকে আরও নিখুঁত ও প্রোডাকশন-রেডি করার অ্যাডভান্সড সোপান নিয়ে আলোচনা করব: **Chapter 14: Advanced Retrieval, Hybrid Search & Re-ranking**। HyDE, প্যারেন্ট-Document রিট্রিভাল, BM25 স্পার্স Search ও রির‍্যাঙ্কিং কীভাবে আরএজি-র এক্যুরেসি ৯৯%-এ নিয়ে যায়, তা আমরা বিস্তারিত শিখব।
