...
আমরা যখন AI-কে বলি আমাদের কাছে অমুক অমুক টুল আছে, তখন আমরা তাকে একটি নির্দিষ্ট JSON Schema ফরম্যাটে টুলের প্রোপার্টি ডিফাইন করে দিই। এটি AI-কে ডিক্টেট করে সে Parameter-এর স্পেলিং বা টাইপ কী জেনারেট করবে।
...
AI যদি একই ইউজার মেসেজে একাধিক কাজ করতে চায়, তবে সে একই রেসপন্সে একাধিক প্যারালাল টুল রিকোয়েস্ট জেনারেট করে:
...
ইনফারেন্সে AI মাঝে মাঝে কড়া JSON Schema কনট্র্যাক্ট ভেঙে ভুল আর্গুমেন্ট টাইপ (যেমন: `threshold_ms` এর জায়গায় স্ট্রিং `"500ms"`) জেনারেট করে ফেলে। 
...
* **সমাধান:** প্রোডাকশন ব্যাকঅ্যান্ডে AI-এর জেনারেট করা আর্গুমেন্ট সরাসরি ফাংশনে পাস করার আগে কঠোরভাবে **Pydantic** বা **Zod (TypeScript)** দিয়ে টাইপ Validation চেক করা বাধ্যতামূলক। যদি Validation ফেল করে, System সাথে সাথে AI-কে Error ফিডব্যাক পাঠিয়ে আবার ট্রাই করতে বাধ্য করে (Self-correction loop)।
...
**ভুল ধারণা:** AI যখন টুল ব্যবহার করে, সে নিজে থেকেই ইন্টারনেটে গিয়ে তোমার থার্ড-পার্টি API কল করে ডিরেক্ট Data রিসিভ করে।
...
চলো পাইথনে কাস্টম JSON Schema ভ্যালিডেটর ব্যবহার করে একটি মিনি সেফ-কমান্ড রানার এজেন্ট তৈরি করি, যা AI-এর আর্গুমেন্ট চেক করে শুধুমাত্র নিরাপদ উইন্ডোজ File লিস্টার টুল এক্সেকিউট করতে পারে।
...
#### Code Breakdown:
* **Input:** AI-এর জেনারেট করা কাস্টম JSON আর্গুমেন্ট টেক্সট।
...
1. **প্রশ্ন:** AI টুল কলিং প্রসেসে JSON Schema-র গুরুত্ব কী?
   * **উত্তর:** JSON Schema হলো AI এবং ব্যাকএন্ড Function-এর মধ্যকার একটি Data কনট্র্যাক্ট (Contract)। এটি পড়ে AI নিখুঁতভাবে সিদ্ধান্ত নেয় ফাংশনে কী টাইপের (যেমন: স্ট্রিং, ইনটেজার) Parameter এবং কী স্পেলিংয়ের আর্গুমেন্ট জেনারেট করতে হবে, যা Code ক্র্যাশ হওয়া রোধ করে।

#### Intermediate
2. **প্রশ্ন:** AI যখন একই রেসপন্সে একাধিক প্যারালাল টুল কল রিকোয়েস্ট করে, তখন ব্যাকএন্ডে কীভাবে তা হ্যান্ডেল করা উচিত?
   * **উত্তর:** ব্যাকএন্ডে রিসিভ করা প্রতিটি টুল কল অবজেক্টের জন্য Loop চালিয়ে আলাদা থ্রেড বা অ্যাসিনক্রোনাসলি (`async/await`) ফাংশনগুলো রান করতে হবে। সবগুলোর রেজাল্ট আলাদা আলাদা ইউনিক `tool_use_id` ট্যাগ দিয়ে মেসেজ এরে-তে প্যাক করে পরবর্তী টার্নে AI-কে ফিড করতে হবে।
...
### ১১. Chapter Summary
* **Tool Calling** অন্ধ AI মডেলকে এক্সটার্নাল API ও Database রিড করার ক্ষমতা দেয়।
...
### ১২. What's Next
দারুণ! আমরা সফলভাবে টুল কলিং এবং ফাংশনাল ইন্টিগ্রেশন প্রসেস জয় করে ফেলেছি। পরবর্তী চ্যাপ্টারে আমরা এই টুলের ইউনিভার্সাল স্ট্যান্ডার্ড প্রোটোকল নিয়ে আলোচনা করব: **Chapter 20: Model Context Protocol (MCP) — The USB-C of AI**। অ্যানথ্রপিকের তৈরি করা মডার্ন এমসিপি (MCP) প্রোটোকল কীভাবে AI টুল ও Resource-এর বৈশ্বিক কানেকশন সহজ করে, তা আমরা বিস্তারিত শিখব।
