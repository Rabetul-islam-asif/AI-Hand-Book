...
### Chapter Goal
এই চ্যাপ্টারের মূল লক্ষ্য হলো অন্ধ এআই মডেলকে চোখ ও হাত দেওয়ার প্রধান গেটওয়ে—অর্থাৎ টুল কলিং (Tool Calling) এবং ফাংশনাল ইন্টিগ্রেশন (Function Integration) এর কোডিং আর্কিটেকচার সম্পূর্ণ আয়ত্ত করা। তুমি জানতে পারবে কীভাবে JSON Schema ব্যবহার করে এআই-এর সাথে একটি নিখুঁত ডেটা কনট্র্যাক্ট (Contract) ডিজাইন করা হয়, এআই কীভাবে ডায়নামিকালি সঠিক আর্গুমেন্ট প্রেডিক্ট করে এবং তোমার ব্যাকএন্ড কোড কীভাবে সেই এআই-এর রিকোয়েস্ট এক্সেকিউট করে চ্যাটলুপ সম্পন্ন করে।

### Why Should I Care?
লার্জ ল্যাঙ্গুয়েজ মডেল (LLM) একা কিন্তু সম্পূর্ণ অন্ধ। সে তোমার ডাটাবেস পড়তে পারে না, ইউজারকে মেসেজ পাঠাতে পারে না এবং আজকের আবহাওয়া কেমন তাও জানে না। তোমার ব্যাকএন্ডের ডাটাবলিং ক্ষমতা এবং থার্ড-পার্টি এপিআই (যেমন: bKash, Pathao APIs) মডেলে ইন্টিগ্রেট করতে হলে তোমাকে টুল কলিং জানতে হবে। এটি এআই চ্যাট ইন্টারফেসকে একটি ডায়নামিক এন্টারপ্রাইজ প্রডাক্টে রূপান্তর করার এক নম্বর হাতিয়ার।
...
* **The Workflow:** বিজ্ঞানী যখনই কোনো জটিল কাজ করতে চান, তিনি মুখের কথায় রোবটের জন্য একটি নিখুঁত ইনস্ট্রাকশন তৈরি করো: `"সিলিন্ডার থেকে ৫ গ্রাম লিকুইড এ নিয়ে টেস্টটিউব বি-তে ঢালো"` (JSON Schema Contract)। তোমার ব্যাকঅ্যান্ড সিস্টেম বা রোবটের কন্ট্রোলার সেই নির্দেশটি রিসিভ করে নিজে কাজটি করে বিজ্ঞানীকে বলে: `"টেস্টটিউব বি-তে ৫ গ্রাম লিকুইড যুক্ত করা হয়েছে"` (Observation/Result)। বিজ্ঞানী সেই রেজাল্ট নিয়ে আবার তার পরবর্তী গবেষণা শুরু করো।

এআই টুল কলিং ঠিক এই প্রসেসে কাজ করে। **এলএলএম নিজে কোনো এপিআই কল বা ডেটাবেস কোয়েরি করতে পারে না।** সে শুধু তোমার সিস্টেমকে গাইড করে কোন ফাংশনটি কী প্যারামিটার দিয়ে রান করতে হবে।
...
রিয়েল কোড ফ্লো-তে ব্যাকএন্ড ডেভেলপার হিসেবে তোমাকে ৪টি ধাপ মেইনটেইন করতে হয়:
...
4. **Friendly Response:** মডেল কাস্টমারকে মিষ্টি বাংলায় বলে: `"ধন্যবাদ, তোমার ৫০০ টাকার পেমেন্টটি সফলভাবে ভেরিফাইড হয়েছে এবং অ্যাকাউন্টে যোগ করা হয়েছে।"`
...
def handle_ai_response(response_obj):
    if response_obj["stop_reason"] == "tool_use":
        ...
        if tool_name == "get_slow_queries":
            ...
            # ৫. রেসপন্স রিম্যাপ করে পরবর্তী টার্নের জন্য প্রিপেয়ার করো
            tool_result_message = {
                "role": "user",
                "content": [
                    {
...
* **The Risk:** এই ভুল প্যারামিটার সরাসরি তোমার ডাটাবেস বা থার্ড-পার্টি ফাংশনে পাস করলে অ্যাপ্লিকেশন এরর বা ডাটাবেস এক্সেপশন ক্র্যাশ করবে।
...
**ভুল ধারণা:** এআই যখন টুল ব্যবহার করে, সে নিজে থেকেই ইন্টারনেটে গিয়ে তোমার থার্ড-পার্টি এপিআই কল করে ডিরেক্ট ডেটা রিসিভ করে।

**বাস্তবতা:** এআই প্রসেস লাইফসাইকেলে কোনো নেটওয়ার্ক রিকোয়েস্ট করতে পারে না। সে শুধু টেক্সট জেনারেট করে। এপিআই-এর সম্পূর্ণ নেটওয়ার্ক কল, অথেনটিকেশন এবং সিকিউরিটি টোকেন হ্যান্ডেল করার দায়িত্ব তোমার ব্যাকএন্ড কোডের। এআই শুধুমাত্র ডিসিশন মেকার।
...
চলো পাইথনে কাস্টম JSON Schema ভ্যালিডেটর ব্যবহার করে একটি মিনি সেফ-কমান্ড রানার এজেন্ট তৈরি করি, যা এআই-এর আর্গুমেন্ট চেক করে শুধুমাত্র নিরাপদ উইন্ডোজ ফাইল লিস্টার টুল এক্সেকিউট করতে পারে।
