...
### Chapter Goal
এই চ্যাপ্টারের মূল লক্ষ্য হলো অন্ধ এআই মডেলকে চোখ ও হাত দেওয়ার প্রধান গেটওয়ে—অর্থাৎ টুল কলিং (Tool Calling) এবং ফাংশনাল ইন্টিগ্রেশন (Function Integration) এর Coding Architecture সম্পূর্ণ আয়ত্ত করা। তুমি জানতে পারবে কীভাবে JSON Schema ব্যবহার করে এআই-এর সাথে একটি নিখুঁত Data কনট্র্যাক্ট (Contract) ডিজাইন করা হয়, এআই কীভাবে ডায়নামিকালি সঠিক আর্গুমেন্ট প্রেডিক্ট করে এবং তোমার ব্যাকএন্ড Code কীভাবে সেই এআই-এর রিকোয়েস্ট এক্সেকিউট করে চ্যাটলুপ সম্পন্ন করে।

### Why Should I Care?
লার্জ ল্যাঙ্গুয়েজ Model (LLM) একা কিন্তু সম্পূর্ণ অন্ধ। সে তোমার Database পড়তে পারে না, ইউজারকে মেসেজ পাঠাতে পারে না এবং আজকের আবহাওয়া কেমন তাও জানে না। তোমার ব্যাকএন্ডের ডাটাবলিং ক্ষমতা এবং থার্ড-পার্টি API (যেমন: bKash, Pathao APIs) মডেলে ইন্টিগ্রেট করতে হলে তোমাকে টুল কলিং জানতে হবে। এটি এআই চ্যাট ইন্টারফেসকে একটি ডায়নামিক এন্টারপ্রাইজ প্রডাক্টে রূপান্তর করার এক নম্বর হাতিয়ার।

### Big Picture
আগের চ্যাপ্টারে আমরা এআই এজেন্টের থিওরি এবং ReAct Loop-এর বেসিক শিখেছি। এই চ্যাপ্টারে আমরা সেই এজেন্ট ইঞ্জিনের আসল হার্ডওয়্যার—অর্থাৎ কীভাবে টুলস ডিজাইন করতে হয় এবং কোডে কীভাবে সেই Loop ঘুরবে তার ফিজিক্যাল ইমপ্লিমেন্টেশন সম্পূর্ণ করব। এটি আমাদের পরবর্তী চ্যাপ্টারের বিশ্বমানের ওপেন-স্ট্যান্ডার্ড **Model Context Protocol (MCP)** বোঝার কোর ফাউন্ডেশন।
...
* **The Interface:** বিজ্ঞানীকে সাহায্য করার জন্য তুমি রুমে একটি কাস্টম রোবোটিক আর্ম বা হাত (Tools) সেটআপ করলে।
...
* **The Workflow:** বিজ্ঞানী যখনই কোনো জটিল কাজ করতে চান, তিনি মুখের কথায় রোবটের জন্য একটি নিখুঁত Instruction তৈরি করো: `"সিলিন্ডার থেকে ৫ গ্রাম লিকুইড এ নিয়ে টেস্টটিউব বি-তে ঢালো"` (JSON Schema Contract)। তোমার ব্যাকঅ্যান্ড System বা রোবটের কন্ট্রোলার সেই নির্দেশটি রিসিভ করে নিজে কাজটি করে বিজ্ঞানীকে বলে: `"টেস্টটিউব বি-তে ৫ গ্রাম লিকুইড যুক্ত করা হয়েছে"` (Observation/Result)। বিজ্ঞানী সেই রেজাল্ট নিয়ে আবার তার পরবর্তী গবেষণা শুরু করো।

এআই টুল কলিং ঠিক এই প্রসেসে কাজ করে। **এলএলএম নিজে কোনো API কল বা Database কোয়েরি করতে পারে না।** সে শুধু তোমার সিস্টেমকে গাইড করে কোন ফাংশনটি কী Parameter দিয়ে রান করতে হবে।

---

### ২. Core Concepts: JSON স্কিমা এবং টুল কলিং Loop

একটি পূর্ণাঙ্গ টুল কলিং Mechanism মূলত ৩টি উপাদানের সমন্বয়ে গঠিত:

#### ক. The JSON Schema Contract (টুল ডিফাইন করার চুক্তিপত্র)
আমরা যখন এআই-কে বলি আমাদের কাছে অমুক অমুক টুল আছে, তখন আমরা তাকে একটি নির্দিষ্ট JSON Schema ফরম্যাটে টুলের প্রোপার্টি ডিফাইন করে দিই। এটি এআই-কে ডিক্টেট করে সে Parameter-এর স্পেলিং বা টাইপ কী জেনারেট করবে।
...
**Description is Code!**  
টুলের ডেসক্রিপশন এবং প্রোপার্টির ডেসক্রিপশন মূলত এআই রিড করে সিদ্ধান্ত নেয় কখন কোন টুল কল করতে হবে। ডেসক্রিপশন অস্পষ্ট হলে (যেমন: শুধু `"Get balance"` লিখলে) এআই ভুলভাল Parameter জেনারেট করে System ক্র্যাশ করবে।
...
3. **Execution:** bKash কাস্টম ব্যাকঅ্যান্ড API কল করে ট্রানজ্যাকশন স্ট্যাটাস `"Success"` এবং এমাউন্ট `"500 BDT"` মেপে মডেলকে Response দেয়।
4. **Friendly Response:** Model কাস্টমারকে মিষ্টি বাংলায় বলে: `"ধন্যবাদ, তোমার ৫০০ টাকার পেমেন্টটি সফলভাবে ভেরিফাইড হয়েছে এবং অ্যাকাউন্টে যোগ করা হয়েছে।"`
...
Developer হিসেবে পাইথনে অ্যানথ্রপিক (Anthropic Claude API) স্ট্যান্ডার্ড ব্যবহার করে একটি সম্পূর্ণ টুল কলিং ইভেন্ট হ্যান্ডলার ইমপ্লিমেন্ট করার রিয়েল ও গোল্ড স্ট্যান্ডার্ড প্রোডাকশন Code:
...
# ৩. মক এআই Response হ্যান্ডলার (Simulating stop_reason='tool_use')
...
* **সমাধান:** প্রোডাকশন ব্যাকঅ্যান্ডে এআই-এর জেনারেট করা আর্গুমেন্ট সরাসরি ফাংশনে পাস করার আগে কঠোরভাবে **Pydantic** বা **Zod (TypeScript)** দিয়ে টাইপ Validation চেক করা বাধ্যতামূলক। যদি Validation ফেল করে, System সাথে সাথে এআই-কে Error ফিডব্যাক পাঠিয়ে আবার ট্রাই করতে বাধ্য করে (Self-correction loop)।
...
**ভুল ধারণা:** এআই যখন টুল ব্যবহার করে, সে নিজে থেকেই ইন্টারনেটে গিয়ে তোমার থার্ড-পার্টি API কল করে ডিরেক্ট Data রিসিভ করে।

**বাস্তবতা:** এআই প্রসেস লাইফসাইকেলে কোনো নেটওয়ার্ক রিকোয়েস্ট করতে পারে না। সে শুধু টেক্সট জেনারেট করে। API-এর সম্পূর্ণ নেটওয়ার্ক কল, অথেনটিকেশন এবং সিকিউরিটি Token হ্যান্ডেল করার দায়িত্ব তোমার ব্যাকএন্ড Code-এর। এআই শুধুমাত্র ডিসিশন মেকার।
...
* **Your Code = ক্যাশিয়ার বা টেলার (The Executor):**
  ম্যানেজার যখন চিরকুটে লিখে ক্যাশিয়ারকে বলো: `"রহিমের অ্যাকাউন্টের ব্যালেন্স মেপে আমাকে জানাও"`, ক্যাশিয়ার কম্পিউটারে কোয়েরি করে ব্যালেন্স মেপে ম্যানেজারকে আবার চিরকুট ফেরত দেয়। ম্যানেজার সেই চিরকুট পড়ে চূড়ান্ত এপ্রুভাল সিগনেচার দেন।

---

### ৯. Mini Project: পাইথনে স্ক্র্যাচ থেকে একটি কাস্টম উইন্ডোজ কমান্ড এক্সিকিউশন সিকিউর এজেন্ট

চলো পাইথনে কাস্টম JSON Schema ভ্যালিডেটর ব্যবহার করে একটি মিনি সেফ-কমান্ড রানার এজেন্ট তৈরি করি, যা এআই-এর আর্গুমেন্ট চেক করে শুধুমাত্র নিরাপদ উইন্ডোজ File লিস্টার টুল এক্সেকিউট করতে পারে।
...
1. **প্রশ্ন:** এআই টুল কলিং প্রসেসে JSON Schema-র গুরুত্ব কী?
   * **উত্তর:** JSON Schema হলো এআই এবং ব্যাকএন্ড Function-এর মধ্যকার একটি Data কনট্র্যাক্ট (Contract)। এটি পড়ে এআই নিখুঁতভাবে সিদ্ধান্ত নেয় ফাংশনে কী টাইপের (যেমন: স্ট্রিং, ইনটেজার) Parameter এবং কী স্পেলিংয়ের আর্গুমেন্ট জেনারেট করতে হবে, যা Code ক্র্যাশ হওয়া রোধ করে।

#### Intermediate
2. **প্রশ্ন:** এআই যখন একই রেসপন্সে একাধিক প্যারালাল টুল কল রিকোয়েস্ট করে, তখন ব্যাকএন্ডে কীভাবে তা হ্যান্ডেল করা উচিত?
   * **উত্তর:** ব্যাকএন্ডে রিসিভ করা প্রতিটি টুল কল অবজেক্টের জন্য Loop চালিয়ে আলাদা থ্রেড বা অ্যাসিনক্রোনাসলি (`async/await`) ফাংশনগুলো রান করতে হবে। সবগুলোর রেজাল্ট আলাদা আলাদা ইউনিক `tool_use_id` ট্যাগ দিয়ে মেসেজ এরে-তে প্যাক করে পরবর্তী টার্নে এআই-কে ফিড করতে হবে।

#### Advanced
3. **প্রশ্ন:** এআই-এর জেনারেট করা আর্গুমেন্ট সরাসরি লোকাল Database ফাংশনে পাস করার প্রোডাকশন রিস্কগুলো কী কী এবং এর বেস্ট প্র্যাকটিস সমাধান কী?
   * **উত্তর:** প্রধান রিস্ক হলো টাইপ মিসম্যাচ এবং কাস্টম Code ইনজেকশন/পাথ ট্রাভার্সাল হ্যাকিং। এর সমাধান হলো সরাসরি Parameter পাস না করে প্রথমে **Pydantic** বা **Zod** এর মতো কড়া স্কিমা ভ্যালিডেটর রান করা এবং Input স্যানিটাইজেশন ফিল্টার নিশ্চিত করার পর শুধুমাত্র ক্লিন Data লোকাল ডাটাবেসে সাবমিট করা।

---

### ১১. Chapter Summary
* **Tool Calling** অন্ধ এআই মডেলকে এক্সটার্নাল API ও Database রিড করার ক্ষমতা দেয়।
* **JSON Schema** এআই-এর Parameter জেনারেশন গাইড করার একমাত্র রুল বুক।
* **Parallel Calling** একই ইউজার মেসেজে একাধিক কাস্টম API কাজ সম্পাদন বুস্ট করে।
