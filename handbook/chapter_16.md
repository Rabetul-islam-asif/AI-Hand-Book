...
### Chapter Goal
এই চ্যাপ্টারের মূল লক্ষ্য হলো লার্জ Language Model (LLM) Fine-Tuning-এর সবচেয়ে saveী এবং revolutionary মেথড— মানে PEFT (Parameter-Efficient Fine-Tuning), LoRA (Low-Rank Adaptation) এবং QLoRA (Quantized LoRA) এর Math-এর ও Practical Intuition ডি-কনস্ট্রাক্ট করা। তুমি জানতে পারবে কীভাবে লিনিয়ার অ্যালজেব্রার Matrix ফ্যাক্টরাইজেশন (Matrix Factorization) ব্যবহার করে Compute কস্ট ও মেমরি প্রায় ৯৫% সেভ করা হয় এবং ৪-বিট Quantization (NF4) কীভাবে কনজিউমার ল্যাপটপের GPU-তেই বিলিয়ন Parameter-এর Model ফাইন-টিউন করতে সাহায্য করে।

### Why Should I Care?
একটি ৭ বিলিয়ন Parameter-এর Model (যেমন: LLaMA-3-8B) ফুল-Parameter Fine-Tuning করতে কমপক্ষে ১৬০ গিগাবাইট GPU ভির‍্যাম (VRAM - প্রায় ৪টি A100 GPU) প্রয়োজন হয়, যা খুব ব্যয়বহুল। কিন্তু LoRA এবং QLoRA প্রযুক্তি আসার পর তুমি মাত্র ১৬ গিগাবাইট VRAM-এর একটি সিঙ্গেল আরটিএক্স (RTX 3090/4090) গ্রাফিক্স কার্ডের সাহায্যে নিজের পার্সোনাল Computeরেই কয়েক ঘণ্টায় একই মানের Fine-Tuning সম্পন্ন করতে পারবে। এই বাজেট-সেভিং টেকনিক AI Engineerিংয়ের সবচেয়ে ডিমান্ডিং স্কিল।

### Big Picture
আগের চ্যাপ্টারে আমরা Custom Fine-Tuning-এর Data প্রিপারেশন এবং এসএফটি (SFT) পাইপলাইন শিখেছি। এই চ্যাপ্টারে আমরা সেই পাইপলাইনকে মিলিয়ন ডলার এন্টারপ্রাইজ ইনফ্রাস্ট্রাকচার থেকে তোমার পার্সোনাল ল্যাপটপে নামিয়ে আনার GPU অপ্টিমাইজেশন Mechanism সম্পূর্ণ করব। এটি আমাদের পরের chapter-এর সেফটি এলাইনমেন্ট (RLHF/DPO) এবং Agentic AI (Agentic AI) বুঝার মূল ভিত্তি।
