...
### Chapter Goal
এই চ্যাপ্টারের মূল লক্ষ্য হলো সাধারণ আরএজি (RAG) পাইপলাইনের এক্যুরেসি ৯০% থেকে ৯৯% এ উন্নীত করার জন্য সবচেয়ে শক্তিশালী প্রোডাকশন Mechanism— মানে অ্যাডভান্সড রিট্রিভাল (Advanced Retrieval), হাইব্রিড Search (Hybrid Search - BM25 + Dense Vector), এবং রির‍্যাঙ্কিং (Re-ranking) এর Architectural Mechanism ক্র্যাক করা। তুমি জানতে পারবে কীভাবে প্যারেন্ট-Document রিট্রিভাল (Parent-Document Retrieval) এবং HyDE (Hypothetical Document Embeddings) কাজ করে এবং প্রোডাকশন প্রজেক্টে এগুলোর perfect সিলেকশন ও Implementation Criteria আয়ত্ত করতে পারবে।

### Why Should I Care?
বাস্তবে দেখা যায়, শুধুমাত্র বেসিক কোসাইন Vector Search ব্যবহার করলে Model অনেক সময় কুয়্যারির প্রাসঙ্গিক বা perfect Document রিট্রাইভ করতে ব্যর্থ হয় (যেমন: নামের বানান ভুল বা Custom পার্ট নাম্বার খোঁজার সময়)। হাইব্রিড Search কীওয়ার্ড এবং মিনিং দুটিই একসাথে Search করে এবং রির‍্যাঙ্কার ভুল রিট্রাইভাল ডকগুলোকে Instantly ফিল্টার করে পারফেক্ট ৩টি Document Promptে পাঠায়। এটি প্রোডাকশন আরএজি ফেইলিউর কমানোর এক নম্বর হাতিয়ার।

### Big Picture
আগের চ্যাপ্টারে আমরা আরএজি-র বেসিক চাংকিং ও ইনজেস্ট পাইপলাইন শিখেছি। এই চ্যাপ্টারে আমরা সেই পাইপলাইনকে এন্টারপ্রাইজ লেভেলের perfect Search ইঞ্জিনে convert করব। এখানে শেখা অপ্টিমাইজেশন আমাদের পরের chapter-এর Custom Model Fine-Tuning (Fine-Tuning) এবং আরএজি বনাম Fine-Tuning-এর Architectural সিদ্ধান্ত নেওয়ার মূল ভিত্তি।

---

### ১. Hook: লাইব্রেরিয়ানের সেরা পৃষ্ঠা ও পুরো বইয়ের সমন্বয়

কল্পনা করো, তুমি লাইব্রেরিতে গিয়ে লাইব্রেরিয়ানকে বললে, **"বিকাশের Dynamic পিন ব্লক কেন হয়?"** 
* **Basic RAG (বেসিক Vector Search):** লাইব্রেরিয়ান বই ঘেঁটে ১টি নির্দিষ্ট Paragraph খুঁজে পেলে যা তোমার কোশ্চেনের সাথে ম্যাচ করে। কিন্তু সমস্যা হলো, সেই প্যারাগ্রাফের ভেতর লেখা আছে: *"অনুচ্ছেদ ৪ এর নিয়ম অনুসারে এটি লক হবে।"* এখন অনুচ্ছেদ ৪ কী? লাইব্রেরিয়ান তা জানো না! ফলে AI মডেলটি ইনকমপ্লিট বা ভুল উত্তর দেবে।
...
* **Parent-Document Retrieval:** লাইব্রেরিয়ান Vector সার্চের মাধ্যমে ছোট চাঙ্কটি খুঁজে পেলে, কিন্তু Promptে পাঠানোর আগে তিনি সেই ছোট চাঙ্কের মূল উৎস বা পুরো "অনুচ্ছেদ ৪"-এর ৪টি Paragraph একসাথে টেনে আনলেন (Parent Document)। এর ফলে AI Model পুরো ব্যাকগ্রাউন্ড জেনে perfect উত্তর দেবে।
...
1. **Sparse Search (BM25):** এটি ফাস্ট কিওয়ার্ড বা Token ফ্রিকোয়েন্সি মেপে Search করে। (যেমন: Customার পার্ট নাম্বার `"bKash-1234"` টাইপ করলে সিমান্টিক Vector হয়তো মিস করবে, কিন্তু BM25 হুবহু কিওয়ার্ড ম্যাচ করে দেবে)।
2. **Dense Search (Vector):** এটি অর্থ বা মিনিং মেপে Search করে।
3. **RRF (Reciprocal Rank Fusion):** এই Algorithmটি স্পার্স এবং ডেন্স সার্চের র্যাঙ্কিং স্কোর একত্রিত করে একটি একক ট্রাস্টেড র্যাঙ্ক স্কোর Produce করে।

#### খ. Re-ranking (রের‍্যাঙ্কিং)
Vector Search Database থেকে টপ ১০টি সম্ভাব্য Document তুলে আনে। কিন্তু তাদের অর্ডারিং বা র্যাঙ্কিং ১০০% perfect নাও হতে পারে।
* **Mechanism:** আমরা একটি বিশেষায়িত **Cross-Encoder Reranker Model** (যেমন: Cohere Rerank বা BGE-Reranker) ব্যবহার করি। 
* এটি ইউজার কোশ্চেন এবং ১০টি ডকের প্রতিটি জোড়া আলাদা আলাদাভাবে ডিপ স্ক্যান করে পরম প্রাসঙ্গিকতা স্কোর পরিমাপ করে পুনরায় শর্টলিস্ট করে বেস্ট ৩টি ডক Promptে পাঠায়। এটি Hallucination প্রায় ৯০% কমিয়ে দেয়।
...
* এবার সেই কাল্পনিক উত্তরটি এম্বেড করে Vector ডাটাবেসে Search করা হয়। যেহেতু কাল্পনিক উত্তর এবং আসল Database-এর ডকের Format ও ডিকশন হুবহু মিলে যায়, তাই রিট্রিভাল Quality Drastically বুস্ট করে।
...
ক্রস-Encoderের কাজ করার perfect Math-এর প্রসেসটি নিচে Diagramের মাধ্যমে ভিজ্যুয়ালাইজ করো:
...
ক্রস-Encoder রির‍্যাঙ্কার মডেলগুলো খুব Computationally হেভি এবং ধীরগতির হয়। প্রোডাকশনে Latency ও কস্ট কন্ট্রোল করতে **Two-Stage Retrieval** ফ্রেমওয়ার্ক ব্যবহার করা বাধ্যতামূলক।
...
**ভুল ধারণা:** রির‍্যাঙ্কার Model যেহেতু খুব নির্ভুল, তাই Database-এর সব লাখ লাখ Document-এর ওপর সরাসরি রির‍্যাঙ্ক রান করা উচিত।
...
চলো পাইথনে Custom NumPy ব্যবহার করে কোনো Library ছাড়া একটি পুরো মিনি হাইব্রিড Search এগ্রিগেটর এবং রির‍্যাঙ্কিং স্কোরার System স্ক্র্যাচ থেকে Architect করি।
...
2. **প্রশ্ন:** টু-স্টেজ রিট্রিভাল (Two-Stage Retrieval) প্রোডাকশনে Latency অপ্টিমাইজেশনে কীভাবে সাহায্য করে?
   * **উত্তর:** এটি প্রথম স্টেজে ফাস্ট Vector Search (HNSW) রান করে মিলি-সেকেন্ডে লক্ষাধিক ডক থেকে ৫০টি ক্যান্ডিডেট শর্টলিস্ট করে। দ্বিতীয় স্টেজে শুধুমাত্র এই ৫০টি ডকের ওপর Computationally হেভি ও ধীরগতির ক্রস-Encoder রির‍্যাঙ্কার রান করে সেরা ৩টি ডক সিলেক্ট করে, যা Search স্পিড ও এক্যুরেসির বেস্ট প্রোডাকশন কম্বিনেশন দেয়।
