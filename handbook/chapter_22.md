...
### Chapter Goal
এই চ্যাপ্টারের মূল লক্ষ্য হলো প্রোডাকশন এআই সিস্টেমের প্রতিটি মিলি-সেকেন্ডের কম্পিউটেশন ও ট্র্যাকিং—অর্থাৎ এআই অবজারভেবিলিটি (AI Observability), ট্র্যাকিং (Tracing - LangSmith), কস্ট ও ল্যাটেন্সি ট্র্যাকিং (Cost & Latency Tracking), এবং রিট্রিভাল কোয়ালিটি পরিমাপের জন্য ইভালুয়েশন ফ্রেমওয়ার্ক (Evaluation Framework - Ragas) এর টেকনিক্যাল আর্কিটেকচার সম্পন্ন করা। চ্যাপ্টারটি শেষ করার পর তুমি নিজেই একটি এডভান্সড অবজারভেবিলিটি ড্যাশবোর্ড ইন্টিগ্রেট করতে পারবে এবং প্রতিটি কাস্টমার কুয়্যারির পেছনের টোকেন খরচ ও এরর স্পট সনাক্ত করতে সক্ষম হবে।

### Why Should I Care?
একটি সাধারণ চ্যাট বা আরএজি অ্যাপ ল্যাবে চমৎকার চলে, কিন্তু প্রোডাকশনে যাওয়ার সাথে সাথেই কাস্টমার কমপ্লেইন করা শুরু করে, **"চ্যাটবট লোড হতে ১০ সেকেন্ড সময় নিচ্ছে!"** অথবা তোমার সিএফও (CFO) এসে তোমাকে ডেকে বললে, **"আজকে ব্যাকএন্ডে মাত্র ১০০ কাস্টমার রিকোয়েস্টে ৫০০ ডলারের এপিআই বিল কীভাবে আসলো?"** তুমি যদি এজেন্টের প্রতি ইটারেশনের ইনার লুপ ট্র্যাক বা ট্রেস (Trace) না করো, তবে কোন টুলে মেমরি লিক হচ্ছে বা কোথায় লুপ ইনফিনিট ঘুরছে, তা তুমি কখনোই বের করতে পারবে না।
...
* **LangSmith / Phoenix:** এগুলো বিশেষায়িত ট্রেসিং হাব। এরা তোমার ব্যাকএন্ড কোডের সাথে ইন্টিগ্রেট হয়ে অটোমেটিক্যালি ইনপুট, আউটপুট, টোকেন সংখ্যা এবং কাস্টম সিস্টেমের ভেতরের ফ্লো চার্ট ড্যাশবোর্ডে পুশ করে দেয়।
...
ট্রেসিং ড্যাশবোর্ডে কীভাবে একটি স্লো রিট্রিভাল এরর পিনপয়েন্ট করা হয়, তা নিচে ডায়াগ্রামের মাধ্যমে ভিজ্যুয়ালাইজ করো:
...
ড্যাশবোর্ডের এই টাইমলাইন চার্ট স্ক্যান করে ডেভেলপার সাথে সাথে বুঝতে পারো ল্যাটেন্সির মূল অপরাধী এলএলএম নয়; অপরাধী হলো ইনডেক্সবিহীন স্লো ভেক্টর ডাটাবেস সার্চ, যা ৮ সেকেন্ড সময় নষ্ট করেছে।
...
* **The Threat:** যদি ইউজার চ্যাটে তার ক্রেডিট কার্ড নম্বর বা পাসওয়ার্ড টাইপ করে এবং তোমার ট্রেসিং লগার হুবহু ইনপুট-আউটপুট ক্লাউড ড্যাশবোর্ডে পাঠিয়ে দেয়, তবে সম্পূর্ণ ডেটা সিকিউরিটি কমপ্লায়েন্স ভেঙে পড়বে (GDPR / HIPAA violations)।
...
**বাস্তবতা:** ট্র্যাকিং বা ট্রেসিং লাইব্রেরিগুলো ব্যাকগ্রাউন্ডে নেটওয়ার্ক রিকোয়েস্ট এবং মেমরি বাফার ব্যবহার করে ক্লাউডে লগ পাঠায়, যা তোমার অ্যাপ্লিকেশনের ওপর অতিরিক্ত প্রসেসিং প্রেসার তৈরি করে (ল্যাটেন্সি সামান্য বাড়াতে পারে)। তাই প্রোডাকশনে সর্বদা **Asynchronous Trace Exporter** (যা প্রধান থ্রেডকে ব্লক না করে ব্যাকগ্রাউন্ডে নন-ব্লকিং কোয়েরি করে) ব্যবহার করা ম্যান্ডেটরি।
...
চলো পাইথনে কাস্টম NumPy ব্যবহার করে কোনো এক্সটার্নাল লাইব্রেরি ছাড়া একটি আরএজি Faithfulness (হ্যালুসিনেশন ডিটেক্টর) ইভালুয়েশন ইঞ্জিন স্ক্র্যাচ থেকে আর্কিটেক্ট করি।
