...
### Chapter Goal
এই চ্যাপ্টারের মূল লক্ষ্য হলো প্রোডাকশন-গ্রেড AI Memory Ecosystem নিজের হাতে Architect করা। আমরা সাধারণ চ্যাটবটের ক্ষণস্থায়ী Memory-এর সীমাবদ্ধতা ভেঙে Redis (ফর সেশন চ্যাট হিস্টোরি) এবং Mem0 (ফর লাইফটাইম ইউজার ফ্যাক্ট Memory) ব্যবহার করে একটি Custom মাল্টি-সেশন চ্যাটবট দাঁড় করাবো, যা ইউজারের দীর্ঘদিনের পছন্দ-অপছন্দ, Project-এর নাম ও ব্যক্তিগত তথ্য সেশন পরিবর্তনের পরেও perfectly মনে রাখতে পারে।

### Why Should I Care?
পুরনো ধাঁচের AI চ্যাটবটগুলোর Memory থাকে কেবল একটি সেশন পর্যন্ত। ইউজার ট্যাব ক্লোজ করলে বা নতুন সেশন শুরু করলে AI সবকিছু ভুলে যায়, যা ইউজারের বিরক্তি বাড়ায়। রিয়েল-ওয়ার্ল্ড AI Assistant (যেমন ChatGPT Plus-এর Custom Memory বা প্রোডাকশন কো-পাইলট) ডিজাইন করতে হলে তোমাকে সেশন Caching ও রিয়েল-টাইম লাইফটাইম ফ্যাক্ট Extractionের হাইব্রিড Architecture রপ্ত করতে হবে।

### Big Picture
এটি আমাদের বাস্তব Project Blueprint লেয়ারের প্রথম মাইলফলক। আগের পার্টগুলোতে আমরা যে এলএলএম সার্ভিং, Prompt Engineerিং এবং Data Vectorাইজেশন থিওরিগুলো শিখেছি, সেগুলোকে রিয়েল-ওয়ার্ল্ড প্রোডাকশন কোডে কনভার্ট করার যাত্রা এখান থেকেই শুরু।
...
* **Layer 1: Short-term Session Memory (Redis):** চ্যাটের শেষ ১০টি মেসেজের রিয়েল-টাইম ফ্লো খুব দ্রুত ক্যাশ করে রাখার জন্য Redis ব্যবহার করা হয়।
* **Layer 2: Long-term Fact Memory (Mem0):** সম্পূর্ণ চ্যাটের ভেতরের সব ফালতু কথা ফিল্টার করে কেবল গুরুত্বপূর্ণ ফ্যাক্ট (যেমন: `{"name": "Rahim", "preference": "Laravel"}`) জেনারেট করে তা Vector Database ও গ্রাফ Architectureে সেভ রাখার জন্য Mem0 ব্যবহার করা হয়। সেশন পরিবর্তন হলেও এই ফ্যাক্টগুলো ব্যাকগ্রাউন্ডে ইনজেক্ট করা হয়।
...
Redis হলো একটি ইন-Memory কী-Value Database।
...
Mem0 (পূর্বে EmbedChain) হলো একটি AI-নেটিভ Memory Ecosystem।
* **Mechanism:** ইউজার যখনই চ্যাট করো, Mem0 ব্যাকগ্রাউন্ডে একটি লাইটওয়েট রিজনার কল করে চ্যাটের ভেতর থেকে কেবল ইউজারের ব্যক্তিগত Persona বা ফ্যাক্ট আলাদা করে।
...
Mem0 ব্যাকগ্রাউন্ডে কীভাবে ফ্যাক্ট Extract ও আপডেট করে তা দেখে নাও:
...
প্রোডাকশন গ্রেড Assistant হোস্ট করার সময় একই সাথে দুই জায়গায় রাইট (Concurrent Writes) করার সময় কনফ্লিক্ট এড়াতে কিছু Condition সেট করতে হয়:
...
চলো NumPy বা পাইথনে কোনো Library ছাড়া একটি স্লাইডিং Window বাফার Code করি যা সেশন হিস্টোরি ৬টি Token সাইজের বেশি হয়ে গেলে প্রথম দিকের মেসেজগুলো Automatically ক্লিয়ার বা স্লাইড করে দেয়।
...
#### Beginner
1. **প্রশ্ন:** একটি প্রোডাকশন চ্যাটবটে "Short-term Memory" এবং "Long-term Memory" এর Practical পার্থক্য কী?
   * **উত্তর:** Short-term memory (যেমন Redis) চ্যাটের লেটেস্ট মেসেজগুলোর রিয়েল-টাইম ফ্লো মনে রাখে যাতে Continuous টকিং perfect হয়। আর Long-term memory (যেমন Mem0) চ্যাটের ভেতরের অপ্রয়োজনীয় টেক্সট বাদ দিয়ে কেবল ব্যক্তিগত তথ্য ও পছন্দ Extract করে Vector ডাটাবেসে forever সংরক্ষণ করে যা সেশন শেষ হওয়ার পরও Dynamically অ্যাক্সেস করা যায়।

#### Intermediate
2. **প্রশ্ন:** কেন Redis-এর চ্যাট ক্যাশে "TTL (Time-to-Live)" সেট করা প্রোডাকশন Architectureে খুব গুরুত্বপূর্ণ?
