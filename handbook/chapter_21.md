...
### Chapter Goal
এই চ্যাপ্টারের মূল লক্ষ্য হলো মডার্ন এআই ইঞ্জিনিয়ারিংয়ের সবচেয়ে অ্যাডভান্সড ডোমেইন—অর্থাৎ হারনেস ইঞ্জিনিয়ারিং (Harness Engineering - `Agent = Model + Harness`) এবং তার ইভালুয়েটর সেন্সর (Evaluator Sensors) মেকানিজম ক্র্যাক করা। তুমি জানতে পারবে কীভাবে প্রজেক্ট রুট ফাইলে `AGENTS.md` ব্যবহার করে এজেন্টের কন্সটিটিউশনাল বাউণ্ডারি সেট করা হয় এবং প্রোবাবিলিস্টিক (Probabilistic) ও ডিটারমিনিস্টিক (Deterministic) গার্ডরেইলের টেকনিক্যাল সমন্বয় ঘটিয়ে এন্টারপ্রাইজ-গ্রেড স্ট্যাবল সিস্টেম আর্কিটেক্ট করতে হয়।

### Why Should I Care?
বাস্তবে দেখা গেছে, **৬৫% এন্টারপ্রাইজ এআই প্রোজেক্টের ফেইলিউরের কারণ মডেলের দুর্বলতা নয়; বরং হারনেস ডিফেক্ট (Harness Defect)।** ডেভেলপাররা মনে করো জিপিটি-৫ আসলে তাদের চ্যাটবটের এরর ঠিক হবে, কিন্তু তাদের ভেতরের ফাইল রিডার বা কোড ভ্যালিডেটর সেন্সর ভাঙা থাকার কারণে এআই প্রতিনিয়ত ভুল ডিসিশন নেয়। কড়া ইভালুয়েশন ও সেন্সর সিস্টেম ডিজাইন করা তোমার প্রডাক্টকে ১০০% নির্ভরযোগ্য করবে।
...
কল্পনা করো, তুমি বাজারে আসা সর্বকালের সবচেয়ে শক্তিশালী রেসিং কারের ইঞ্জিন (যেমন: V12 Twin-Turbo) তোমার গ্যারেজে আনলেন। এটি তোমার বেস এআই মডেল (LLM)।
* **The Mess (ইঞ্জিন একা নিরুপায়):** তুমি স্রেফ চাকা ও ইঞ্জিনের ওপর সিট বসিয়ে হাইওয়েতে স্পিড তুলে দিলে। গাড়িটি সেকেন্ডে ৩০০ কিমি বেগে ছুটলো, কিন্তু তোমার গাড়িতে কোনো স্টিয়ারিং হুইল নেই, কোনো ব্রেক নেই, কোনো সিটবেল্ট নেই এবং এয়ারব্যাগও নেই! প্রথম বাঁকেই গাড়িটি ক্র্যাশ করে তোমাকে পঙ্গু করে দেবে।
...
* **Harness** হলো ওরিজিনাল কোডের চারপাশের নিরাপত্তা ও কন্ট্রোল বডি, যা তোমার প্রডাক্টটিকে প্রোডাকশন-রেডি করে।
...
1. **Constitutional Alignment:** এটি ওপেন হওয়ার সময় তোমার প্রজেক্টের `CLAUDE.md` এবং `AGENTS.md` রিড করে।
2. **Deterministic Safety:** তুমি যখন কোড চেঞ্জ করতে বলো, সে কোড লিখে সেভ করে এবং ব্যাকগ্রাউন্ডে অটোমেটিক্যালি তোমার প্রোজেক্টের `pnpm test` বা `npm run lint` সেন্সর রান করে। টেস্ট ফেইল করলে সে নিজে থেকেই এরর ডিটেক্ট করে কোড রি-রাইট করে ফিক্স করে।
...
ইন্ডাস্ট্রি স্ট্যান্ডার্ড কাস্টম `AGENTS.md` টেমপ্লেট ফাইল যা তোমার ওএস-এ ডিপ্লয় করতে হবে:
...
চলো পাইথনে কাস্টম পাইপলাইন ব্যবহার করে একটি প্রোডাকশন-গ্রেড ৩-ধাপের ক্যাসকেড সেন্সর (Computational -> Unit -> LLM-as-a-Judge) সিস্টেম ডিজাইন করি।
