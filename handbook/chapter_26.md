...
### Chapter Goal
এই চ্যাপ্টারের মূল লক্ষ্য হলো একটি অত্যন্ত শক্তিশালী সায়েন্স-ফিকশন লেভেলের এআই এজেন্ট ইকোসিস্টেম স্বহস্তে তৈরি করা। আমরা রিঅ্যাক্ট (ReAct: Reason + Act) Pattern ব্যবহার করে এমন একটি কমান্ড-লাইন অটোমেটিক Code রাইটার এজেন্ট (Agentic CLI Code Writer) দাঁড় করাবো, যা ইউজারের ইন্সট্রাকশন শুনে তোমার লোকাল কম্পিউটারে Code File তৈরি করবে, নিজেই টার্মিনালে Test স্ক্রিপ্ট রান করবে এবং Test ফেইল করলে ডাইনামিকালি Error বা ট্র্যাকিং Log রিড করে অটোমেটিক্যালি Code এডিট বা সেলফ-হিলিং (Auto-Test Healing) সম্পন্ন করবে।

### Why Should I Care?
গতানুগতিক চ্যাটবট শুধু Code লিখে চ্যাটবক্সে শো করতে পারে। কিন্তু সেই কোডে কোনো সিনট্যাক্স Error আছে কি না বা রিয়েল ডিপেনডেন্সি ক্র্যাশ করছে কি না, তা চ্যাটবট নিজে রান না করে জানতে পারে না। Devin বা Cursor Agent-এর মতো বৈপ্লবিক এআই কো-পাইলট System গড়ার পেছনের মূল রহস্য হলো এই সেলফ-Testing ও হিলিং Loop। একজন এআই Engineer হিসেবে এজেন্ট Loop Architecture রপ্ত করতে পারলে তুমি যেকোনো সাধারণ সফটওয়্যারকে স্বয়ংক্রিয় এআই এজেন্টে রূপান্তর করতে পারবে।

### Big Picture
এটি আমাদের বাস্তব প্রোজেক্ট ব্লুপ্রিন্ট লেয়ারের তৃতীয় এবং সবচেয়ে অ্যাডভান্সড কাইন্ডের ফ্ল্যাগশিপ মাইলফলক। আগের চ্যাপ্টারগুলোতে আমরা থিওরিটিক্যাল টুল কলিং, Function স্কিমাস এবং Conditional Loop শিখেছি। এই চ্যাপ্টারে আমরা সেগুলোকে প্র্যাক্টিক্যাল ডকার বা লোকাল কমান্ড এক্সিকিউট পাইপলাইনে রূপ দেবো।

---

### ১. The Problem: অন্ধ কোডারের ছটফটানি ও Error হিলিং

একটি সাধারণ এআই যখন Code জেনারেট করে:
```python
# AI Generated code
import request  # Oops! It should be 'requests' with an 's'
```
ইউজার যখন Code রান করতে যাও, স্ক্রিনে বড় লাল Error আসে: `ModuleNotFoundError: No module named 'request'`। ইউজারকে আবার সেই Error কপি করে এআই-তে পেস্ট করতে হয়। এই অনবরত কপি-পেস্ট করা ডেভেলপারের মূল্যবান সময় নষ্ট করে।
...
1. **Think (পরিকল্পনা):** এলএলএম Prompt অ্যানালাইসিস করে ডিসিশন নেয় কোন Library বা Code লিখতে হবে।
2. **Write (অ্যাকশন ১):** এজেন্ট তার ওন কাস্টম Code রাইটিং টুল ব্যবহার করে লোকাল কম্পিউটারে `app.py` এবং একটি `test_app.py` File তৈরি করে।
3. **Execute Test (অ্যাকশন ২):** এজেন্ট লোকাল টার্মিনালে সাব-প্রসেস রান করে Test কমান্ডটি এক্সিকিউট করে: `pytest test_app.py`।
4. **Self-Heal (পর্যবেক্ষণ ও ভুল সংশোধন):** Test যদি ফেইল করে, এজেন্ট কিন্তু হাল ছাড়ে না। সে টেস্টের Output ও লাল Error লগটি সরাসরি রিড করে তার ব্রেইনে ফিড করে এবং তার Code সংশোধন করে আবার নতুন করে লেখে। Test ১০০% পাস হওয়া না পর্যন্ত এই Loop অনবরত চলতে থাকে।
...
* **Thought (চিন্তা):** *"আমি একটি ম্যাথমেটিক্যাল ক্যালকুলেটর Function লিখতে চাই। প্রথমে আমার ফাইলটি রাইট করা উচিত।"*
* **Action (কাজ):** কাস্টম Function কল করা (যেমন `write_file_to_disk`).
...
একটি বড় Software কোম্পানির ডাটাবেসে একটি সিকিউরিটি ভালনারেবিলিটি বা বাগ পাওয়া গেল:
1. এআই এজেন্ট স্বয়ংক্রিয়ভাবে সোর্স Code-এর ফাইলটি রিড করে ডকার স্যান্ডবক্সে Code প্যাচ করে।
2. প্যাচ করার পর সে তার সিকিউরিটি Regression Test রান করে দেখে কোনো System ডাউন হয়েছে কি না।
3. টেস্টে যদি দেখা যায় ইউজার লগইন ফেইল করছে, এজেন্ট সাথে সাথে কোডটি আবার রোলব্যাক করে বা মডিফাই করে Test পাস করিয়ে নিমিষেই গিটহাবে পিআর (Pull Request) সাবমিট করে দেয়।
...
চলো পাইথনে একটি সম্পূর্ণ রানিং, কাস্টম সেলফ-হিলিং এজেন্ট Loop স্ক্র্যাচ থেকে ডিজাইন করি যা ফেইলড পাইথন টেস্টকে নিজে নিজেই হিল বা রিসলভ করবে।
...
* **Resource Limiting (কম্পিউট লকিং):** কোনো হ্যাকার যদি প্রম্পটে একটি ইনফিনিটি Loop Code লিখে তোমার সার্ভারে Test রান করায়, তবে তোমার CPU ক্র্যাশ করবে। তাই প্রতিটি সাব-প্রসেস এক্সিকিউশনে অবশ্যই `timeout=10.0` Parameter ব্যবহার করতে হবে যাতে ১০ সেকেন্ডের বেশি কোনো স্ক্রিপ্ট চললে তা সাথে সাথে কিল হয়ে যায়।
...
**ভুল ধারণা:** এজেন্টের সোর্স কোডে `eval()` বা `exec()` Function ব্যবহার করে রানিং পাইথনের ভেতর ডাইনামিকালি এআই জেনারেটেড Code এক্সিকিউট করা।

**বাস্তবতা:** `eval()` ব্যবহার করা পাইথনের সবচেয়ে বড় সিকিউরিটি লিকেজ। এর ফলে রানিং পাইথন প্রসেসের সম্পূর্ণ Memory ও ভেরিয়েবল এআই স্ক্রিপ্ট ডাইনামিকালি ম্যানিপুলেট বা হ্যাক করে ফেলতে পারে। Code সবসময় সম্পূর্ণ আলাদা ফাইলে সেভ করে সাব-প্রসেস হিসেবে হোস্ট করাই একমাত্র প্রোডাকশন-গ্রেড সিদ্ধান্ত।
...
**"আমাদের সেলফ-হিলিং এজেন্ট হলো একজন মিস্ত্রি যে পাইপ বসায়। আর Test রানার হলো সেই খিটখিটে অ্যাসিস্ট্যান্ট যে অনবরত পাইপের ভেতরে জল ঢেলে (Test cases) লিক খোঁজে। যখনই লিক পাওয়া যায়, অ্যাসিস্ট্যান্ট চেঁচিয়ে বলে লিক কোথায়, আর মিস্ত্রি সাথে সাথে প্লাস্টার বা ওয়েল্ডিং করে পাইপ নিখুঁত করে দেয়।"**

---

### IX. Mini Project: পাইথনে কাস্টম সাব-প্রসেস টাইমআউট ট্র্যাপার

চলো পাইথনে Code করে প্র্যাক্টিক্যালি দেখি কীভাবে কোনো কমান্ড বা এআই স্ক্রিপ্ট ইনফিনিটি লুপে আটকে গেলে তাকে টাইমার দিয়ে কিল করতে হয়।
...
#### Beginner
1. **প্রশ্ন:** এআই এজেন্টের প্রসঙ্গে "Reason + Act (ReAct)" Loop বলতে কী বোঝায়?
...
#### Intermediate
2. **প্রশ্ন:** কেন সেলফ-হিলিং এজেন্টে জেনারেটেড Code রান করার সময় `timeout` Parameter দেওয়া অত্যন্ত জরুরি?
   * **উত্তর:** এআই Code লেখার সময় অনেক সময় ভুল করে ইনফিনিটি Loop জেনারেট করে ফেলতে পারে যা টার্মিনালে রান করলে CPU ১০০% এনগেজ করে পুরো Server ক্র্যাশ করিয়ে দেবে। `timeout` Parameter দিলে নির্দিষ্ট সময় (যেমন ১০ সেকেন্ড) পর প্রসেসটি অটো-টার্মিনেট বা কিল হয়, যা System রিসোর্স সেভ করে।

#### Advanced
3. **প্রশ্ন:** প্রোডাকশনে Devin বা Cursor-এর মতো Code জেনারেটর এজেন্ট ডিজাইনে "Docker Sandbox Container" এর প্রয়োজনীয়তা গাণিতিক ও আর্কিটেকচারাল দিক থেকে ব্যাখ্যা করো।
   * **উত্তর:** এআই বা ম্যালিসিয়াস Prompt যদি লোকাল ওএসে সরাসরি অ্যাক্সেস পায়, সে তোমার কম্পিউটারের সব পার্সোনাল File রিড করতে পারে বা ক্ষতিকর কমান্ড রান করে Database মুছে দিতে পারে। ডকার স্যান্ডবক্স কন্টেইনার একটি আলাদা লাইটওয়েট ভার্চুয়াল ফাইলসিস্টেম তৈরি করে যার সাথে লোকাল ওএসের কোনো Memory বা নেটওয়ার্ক শেয়ার থাকে না। ফলে এজেন্ট ডকারের ভেতরে কোনো ক্ষতি করলেও কন্টেইনারটি বন্ধ করার সাথে সাথে Database ও System রিবুট হয়ে ফ্রেশ স্টেটে ফিরে আসে, যা লোকাল ওএসকে ১০০% সিকিউর রাখে।

---

### XI. Chapter Summary
* **ReAct Framework** এজেন্টকে মানুষের মতো ধাপে ধাপে চিন্তা ও অ্যাকশন নেওয়ার ক্ষমতা দেয়।
* **Auto-Test Healing** ভুল Error Log রিড করে স্বয়ংক্রিয়ভাবে Code বাগ সংশোধন করে।
* প্রোডাকশন এআই এজেন্ট ডিজাইনের প্রধান শর্ত হলো **Strict Security Isolation (Docker Sandbox)**।

---

### XII. What's Next
আমরা Deep Learning ও এআই ইকোসিস্টেমের কঠিনতম ও রোমাঞ্চকর এজেন্ট Loop Architecture সফলভাবে শেষ করেছি। পরবর্তী চ্যাপ্টারে আমরা পদার্পণ করতে যাচ্ছি আমাদের ফাইনাল বা শেষ টেকনিক্যাল প্রোজেক্ট ব্লুপ্রিন্টে: **Part 11 — Building Real AI Products এর Chapter 27: Blueprint 4 — Production AI SaaS with Rate Limiting & Usage Billing**। কীভাবে একটি বিশ্বমানের এআই সফটওয়্যারকে সাবস্ক্রিপশন বা পেইড সার্ভিসে রূপান্তর করে Redis রেট-লিমিটিং, API Token ব্যবহার ট্র্যাকিং এবং স্ট্রাইপ (Stripe) ইউসেজ-বেসড বিলিং ইন্টিগ্রেশন করা হয়, তা আমরা বিশদ Source Code ও Architecture দিয়ে স্বহস্তে ডিজাইন করবো।
